...
let rec drop_types (t: term): Mini_fun.term =
  ...
  in
  match t with
  | Int n -> Mini_fun.Int n
  | Bool b -> Mini_fun.Bool b
  | Var x -> Mini_fun.Var x
  | Fun (x, _, body) -> Mini_fun.Fun (x, drop_types body)
  | App (f, arg) -> Mini_fun.App (drop_types f, drop_types arg)
  | Op (o, t1, t2) ->
      Mini_fun.Op (cast_op o, drop_types t1, drop_types t2)
  | Not t -> Mini_fun.Not (drop_types t)
  | If (c, t, e) ->
      Mini_fun.If (drop_types c, drop_types t, drop_types e)
  | Let (x, v, b) -> Mini_fun.Let (x, drop_types v, drop_types b)
  | LetFun (f, x, _, body, b) ->
      Mini_fun.LetFun (f, x, drop_types body, drop_types b)
