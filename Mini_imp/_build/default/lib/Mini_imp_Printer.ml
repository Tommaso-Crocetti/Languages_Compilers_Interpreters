...
let rec command_to_string (c : command) : string =
  ...
  | Seq (c1, c2) ->
      "(" ^ command_to_string c1 ^ ") ; (" ^ command_to_string c2 ^ ")"
  | If (b, c1, c2) ->
      "if " ^ bexp_to_string b ^ " then (" ^ command_to_string c1 ^ ") else ("
      ^ command_to_string c2 ^ ")"
  | While (b, c) ->
      "while " ^ bexp_to_string b ^ " do (" ^ command_to_string c ^ ")"
...
let string_of_int_list (lst : int list) : string =
  match lst with
  | [] -> "[]"
  | _ -> "[" ^ String.concat ", " (List.map string_of_int lst) ^ "]"
...
let generic_cfg_to_string (node_to_string : int -> 'a -> string)
    (cfg : 'a generic_cfg) : string =
  let nodes_str =
    String.concat "\n"
      (List.map
         (fun (node_id, node) -> node_to_string node_id node)
  ...
  in
  let nodes_section = if nodes_str = "" then "  (none)" else nodes_str in
  let edges_section = if edges_str = "" then "  (none)" else edges_str in
  let initial_final =
    Printf.sprintf "Initial node: %d\nFinal nodes: %s" cfg.initial
      (string_of_int_list cfg.final)
  in
  initial_final ^ "\nNodes:\n" ^ nodes_section ^ "\nEdges:\n" ^ edges_section
...
let cfg_to_string (cfg : cfg) : string =
  generic_cfg_to_string
    (fun node_id stmts ->
      let stmts_str =
        String.concat "; " (List.map string_of_cfg_statement stmts)
      in
...
let string_of_risc_urop (urop : urop) : string =
  match urop with Not -> "not" | Copy -> "copy"
...
let string_of_risc_instruction (instr : instruction) : string =
  ...
  | Biop (b, r, n, r2) ->
      Printf.sprintf "%s %s %d => %s" (string_of_risc_biop b)
        (string_of_risc_reg r) n (string_of_risc_reg r2)
  | Urop (u, r1, r2) ->
...
let risc_cfg_to_string (cfg : risc_cfg) : string =
  generic_cfg_to_string
    (fun node_id instrs ->
      let instrs_str =
        String.concat "; " (List.map string_of_risc_instruction instrs)
      in
...
let reg_map_to_string (reg_map : var_to_reg) : string =
  String.concat ", "
    (List.map
       (fun (var, reg) ->
         Printf.sprintf "%s -> %s" var (string_of_risc_reg reg))
...
let risc_cfg_and_reg_map_to_string (risc_cfg : risc_cfg)
    (final_reg_map : var_to_reg) : string =
  Printf.sprintf "%s\nFinal Register Map:\n%s"
    (risc_cfg_to_string risc_cfg)
    (reg_map_to_string final_reg_map)

let color_map_to_string (color_map : reg RMap.t) : string =
  if RMap.is_empty color_map then
    "=== Final color map ===\nvirtual reg -> physical reg\n  (empty)"
  else
    let body =
      RMap.bindings color_map
      |> List.map (fun (src, dst) ->
             Printf.sprintf "  %s -> %s" (string_of_risc_reg src)
               (string_of_risc_reg dst))
      |> String.concat "\n"
    in
    "=== Final color map ===\nvirtual reg -> physical reg\n" ^ body

let spilled_registers_to_string (regs : reg_set) : string =
  if RSet.is_empty regs then "No registers were spilled during allocation."
  else
    let entries =
      RSet.elements regs
      |> List.map (fun reg -> Printf.sprintf "  %s" (string_of_risc_reg reg))
      |> String.concat "\n"
    in
