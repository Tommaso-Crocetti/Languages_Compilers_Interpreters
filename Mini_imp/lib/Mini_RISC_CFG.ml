...
let build_risc_cfg (cfg: cfg): risc_cfg * var_to_reg * reg =
  ...
  let risc_nodes = IMap.mapi (fun node_id stmts ->
    ...
    let instructions, _ = translate_stmts stmts temp_regs guard_reg all_vars_reg_map in
    instructions
  ) cfg.nodes in
  { nodes = risc_nodes; edges = cfg.edges; initial = cfg.initial; final = cfg.final; input_var = cfg.input_var; output_var = cfg.output_var; all_vars = cfg.all_vars }, all_vars_reg_map, guard_reg
...
let append_jump (guard_reg: reg) (cfg: risc_cfg) (node_id: int) (instrs: instruction list) : instruction list =
