...
let build_dataflow_cfg (cfg: cfg) : dataflow_cfg =
  let nodes = IMap.map (fun stmts ->{ stmts; in_vars = cfg.all_vars; out_vars = cfg.all_vars }) cfg.nodes in
...
let defined_local_update (df_cfg: dataflow_cfg) (node_id: int) (predecessors: int list) (def_vars: var_set): var_set * var_set =
  let new_in_vars =
...
let defined_global_update (df_cfg: dataflow_cfg) : dataflow_cfg =
  ...
  let def_vars_map = IMap.map (fun df_node -> find_defined_vars df_node.stmts) df_cfg.nodes in
  let initial_worklist = List.map fst (IMap.bindings df_cfg.nodes) in
  let extract_fact (df_node: dataflow_node) : var_set * var_set = (df_node.in_vars, df_node.out_vars) in
  ...
  in let eq (prev_vars: var_set * var_set) (new_vars: var_set * var_set) : bool =
    let (prev_in_vars, _) = prev_vars in
    let (in_vars2, _) = new_vars in
    SSet.equal prev_in_vars in_vars2
  in let update_node (df_cfg: dataflow_cfg) (df_node: dataflow_node) (new_in_out: var_set * var_set) : dataflow_node =
    let (new_in_vars, new_out_vars) = new_in_out in
    { df_node with in_vars = new_in_vars; out_vars = new_out_vars }
  in let successors (node_id: int) : int list =
  ...
  in let update_worklist (worklist: int list) (node_id: int) (successors: int list) : int list =
    List.filter (fun id -> id <> node_id) worklist @ successors
  in visit df_cfg initial_worklist extract_fact local_update eq update_node successors update_worklist
